		...
		foreach ($fetchAllavion as $avion) {
            ...
            if (!empty($maintenance))
            {
                $listeavion[]   = array('nom_model'				=> $modelavion['nom_model']
						            ...
						            , 'heure_depuis_revision' 	=> $avion['heure_depuis_revision']
						            , 'date_prevue' 			=> $maintenance['date_prevue']
						            , 'date_eff' 				=> $maintenance['date_eff']
            ...
            if ($formMaintenance->isValid($formData)) {
					...
					if ($verif == true)
					{
						if ( $datedeb > time() ){//Verifie si on met pas un maintenance avans la date actuel
							...
							echo 'La date de maintenance doit etre superieur à la date actuel !';
						}

					}else{
	            		echo 'Il a déjà une maintance à cette date !';
	            	}
            ...
            if ($formNouveauModel->isValid($formData)) {
				...
				if ($nompispo == true){
					...
					if ($rayon_action > 0 && $longueur_piste > 0 && $nbr_place > 0 && $vitesse > 0){
						...
						echo 'Le rayon d\'action , la longueur de piste , le nombre de places ou la vitesse doivent être superieur à 0.';
					}
				}
				else {
